...
module RANGECODER {
	...
	function EncodeFile(data : number[], alphabet : any) : number[] {
		var io : IO = new IO(data);
		var rc : RangeCoder = new RangeCoder(io);
		io.writeDWORD(data.length);
		var o0c : ORDER_0_CODER = new ORDER_0_CODER(rc, alphabet);
	...
	function DecodeFile(data : number[], alphabet : any) : number[] {
		var io : IO = new IO(data),
			rc : RangeCoder = new RangeCoder(io),
			o0c : ORDER_0_CODER = new ORDER_0_CODER(rc, alphabet),
	...
	function main() {
		...
		var encodedData = EncodeFile(data, alphabet);
		console.log("encoded data: " + encodedData.length + " bytes");

		var decodedData = DecodeFile(encodedData, alphabet);
