...
class HTable {
	constructor(up, down, symbol, weight) {
...
class Huffman {
	...
	//  of nodes to be used
	constructor(size, root, bitstream, max_weight) {
		var i;
	...
	static compressFile(data : number[], size : number) {
	...
	static decompressFile(data : number[], size : number) {
		var alphabetSize = 256;
		if (size < 0) {
			alphabetSize++;
		}
